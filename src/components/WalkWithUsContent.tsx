...
const TABS = ["후원하기", "연간기금 및 활동실적 내역"] as const;
type Tab = (typeof TABS)[number];

/** 연간기금 및 활동실적 내역 리스트 (date + title) */
const ANNUAL = Array.from({ length: 10 }, () => ({ date: "2026.03", title: "코리아둘레길 지킴이 모집" }));
...
export default function WalkWithUsContent() {
  const [tab, setTab] = useState<Tab>("후원하기");
  ...
  return (
    <>
      ...
      <div className="absolute" style={{ left: 0, top: 296, width: 1920, height: 868 }}>
        <Image src="/intro/ww-donate.jpg" alt="후원하기" fill sizes="1920px" className="object-cover object-bottom" />
      </div>
      ...
      <div className="absolute flex flex-col gap-[50px]" style={{ left: 410, top: 892, width: 1100 }}>
        ...
        <div className="flex flex-col items-end gap-[24px] text-white">
          ...
          <div className="w-full" style={{ fontSize: 18, lineHeight: 1.5, letterSpacing: "-0.468px" }}>
            <p>걷기 길의 지속가능한 발전을 지원해주세요.</p>
            <p>모아주신 후원금은 걷기 길의 지속가능한 발전을 위한 다양한 활동에 사용됩니다.</p>
          </div>
        </div>
        ...
        <div className="relative w-full rounded-br-[60px] rounded-tl-[60px] bg-white" style={{ height: 243 }}>
          ...
          <button
            ...
            style={{ left: 456, top: 82, width: 30, height: 30 }}
          >
            {copied ? (
          ...
          {copied && (
            <span
              ...
              style={{ left: 496, top: 80, fontSize: 14, lineHeight: 1.6 }}
            >
              복사됨
            </span>
          )}
          ...
          <ul className="absolute list-disc text-black" style={{ left: 700, top: 105, fontSize: 18, lineHeight: 1.5, letterSpacing: "-0.468px" }}>
            <li className="whitespace-nowrap">걷기 여행길 조성 및 유지보수</li>
            <li className="whitespace-nowrap">걷기 여행길 교육&amp;문화 프로그램 운영</li>
            <li className="whitespace-nowrap">자원봉사 운영등</li>
          </ul>
        </div>
      </div>
    </>
