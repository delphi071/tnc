...
export default function KoriaDulegilSection() {
  ...
  // 라벨 실측 합계 860.8 → 패딩 16 일 때 1084.8 로 1100 안에 들어온다.
  // (Figma 는 마지막 탭이 Certifications 이던 시절 기준이라 20 이었다. 한국어는 짧아 기본값으로 충분)
  ...
  const slides: CarouselSlide[] = t.koriaDulegil.tabs.map((tab, i) => ({
    ...
    // 마지막(완보 인증) 탭만 제목 옆 CTA 버튼
    cta: i === HASH_TABS.certifications ? <CertifyButton label={t.koriaDulegil.certifyCta} /> : undefined,
    blocks: tab.blocks,
  }));
  return (
    <TabCarousel
