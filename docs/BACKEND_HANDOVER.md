...
src/
  app/            # 라우트/페이지 (App Router). 각 폴더 = URL 경로
    our-stories/notices/[id]/   # 공지 상세(동적 라우트)
...
2. **동적 라우트 `/notices/[id]`** — 정적 export 는 빌드 시점에 모든 id 를 알아야 함.
...
|---|---|---|---|---|
| GET | `/api/notices?page={n}` | 공지 목록(페이지네이션) | - | `{ items: [{id,title,body,date}], total, page }` |
...
| 기능 | 파일 | 현재 상태 |
|---|---|---|
| 공지 목록 | `src/components/NoticesSection.tsx` | 샘플 5개 고정 |
| 공지 상세 | `src/components/NoticeDetail.tsx` · 라우트 `src/app/our-stories/notices/[id]` | 샘플 고정, id 미사용 |
| 소식받기 구독 | `src/components/SubscribeSection.tsx` · 모바일 `OurStoriesMobile.tsx` | submit 이 `preventDefault` 만 함 |
| 문의하기 | `src/components/InquirySection.tsx` | 동상 |
| 연간기금 내역 | `src/components/WalkWithUsContent.tsx`(`AnnualBody`) · 모바일 `WalkWithUsMobile.tsx` | 샘플 10개 고정 |
